...
void configModeCallback (WiFiManager *myWiFiManager) {
  u8g2.clearBuffer();
  echoScreen(0, 20, "配网模式");
  echoScreen(0, 40, "AP:"+myWiFiManager->getConfigPortalSSID());
  echoScreen(0, 60, "http://"+WiFi.softAPIP().toString());
  ...
  if ( digitalRead(TRIGGER_PIN) == LOW ) {
    ...
    if( digitalRead(TRIGGER_PIN) == LOW ){
      ...
      if( digitalRead(TRIGGER_PIN) == LOW ){
        ...
        //擦除原来的配置
        wm.resetSettings();
        echoScreen(0, 40, "擦除原有数据");
        echoScreen(0, 60, "设备即将重启");
        delay(800);
...
//配网模式的setup函数，改名为 initWiFi() 不影响天气预报的原调用代码
void initWiFi(){
  
  u8g2.clearBuffer();
  echoScreen(0, 20, "准备联网");
  WiFi.mode(WIFI_STA);
  ...
  if(!res) {
    echoScreen(0, 40, "联网超时");
    Serial.println("Failed to connect or hit timeout");
  } 
  else {
    echoScreen(0, 40, "联网成功");
