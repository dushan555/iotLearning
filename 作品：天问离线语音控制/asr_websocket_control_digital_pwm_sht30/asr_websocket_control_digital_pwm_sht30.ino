...
void loop() {
  ...
  if(current_time - auto_last_time > auto_interval){
    ...
    if(pin_data.temperature>32){  //如果温度超过设定值
      ...
      // 如果D5引脚状态不是高电平，则修改为高电平，并通知客户端
      if(pin_data.d[4] < 1){
        analogWrite(pin_data.dname[4],255);
        pin_data.d[4] = 255;
        is_changed = true;
      }
      ...
      if(pin_data.d[4] != 0){
        analogWrite(pin_data.dname[4],0);
        pin_data.d[4] = 0;
        is_changed = true;
      }      
    }
  ...
  // 目前主要用于天问 ASR 离线语音控制模块
  // 从串口接收数据，并控制D6、7、8 引脚
  // D5 已经用温湿度传感器控制了，就不要接了
  String tmp;
  if(Serial.available()){
    tmp = Serial.readStringUntil('\n');
    Serial.println(tmp);
    DeserializationError error = deserializeJson(cmd, tmp);
    if(!error){
      int p = cmd["p"];
      int v = cmd["v"];
      
      pin_data.d[p] = v;
      analogWrite(pin_data.dname[p],v);
