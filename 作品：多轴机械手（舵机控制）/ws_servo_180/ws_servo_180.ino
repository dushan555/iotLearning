...
struct PINDATA { 
  ...
  const uint8_t servo_count = 5;  //有几个引脚支持舵机？
  const uint8_t servo_pins_index[5] = {0,2,3,4,5};  //舵机引脚的索引！参考dname，可同时控制多个舵机，输入对应引脚的索引即可
  int servo_index[5]  = {-1,-1,-1,-1,-1};  //舵机对象的默认状态
  
  
  const uint8_t timer_index = 5;  //支持定时的引脚索引（对应dname数组的索引，0表示第一个引脚，2表示第三个引脚。）
  uint16_t timer_pwm = 180; //定时器开始工作后，引脚的状态（通过web修改引脚的值以后，会同步更新）
